...
public class ProxyMap {
  ...
  private Map proxies = new TreeMap();
  ...
  public ProxyForwarder add(ProxyForwarder proxyForwarder,
    ...
    return (ProxyForwarder)
        this.proxies.put(new ProxyKey(contextEngineID, proxyType),
                         proxyForwarder);
  }
  ...
  public ProxyForwarder remove(OctetString contextEngineID,
                               int proxyType) {
    return (ProxyForwarder)
        this.proxies.remove(new ProxyKey(contextEngineID, proxyType));
  }

  public ProxyForwarder get(OctetString contextEngineID,
                            int proxyType) {
    ProxyForwarder proxy =(ProxyForwarder)
        this.proxies.get(new ProxyKey(contextEngineID, proxyType));
    if (proxy == null) {
      proxy = (ProxyForwarder) this.proxies.get(new ProxyKey(null, proxyType));
    }
    return proxy;
  }
