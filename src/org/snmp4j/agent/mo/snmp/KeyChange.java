...
/*
    byte[] temp = new byte[keyLength*2];
    System.arraycopy(oldKey.getValue(), 0, temp, 0, keyLength);
    System.arraycopy(keyChange.getValue(), 0, temp, keyLength, keyLength);
    temp = authProtocol.hash(temp);
    byte[] newKey = new byte[keyLength];
    System.arraycopy(temp, 0, newKey, 0, keyLength);
    for (int i=0; i<keyLength; i++) {
      newKey[i] ^= keyChange.get(keyLength+i);
    }
    return new OctetString(newKey);
 */
